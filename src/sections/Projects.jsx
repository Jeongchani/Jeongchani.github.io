...
function Projects({ filter }) {
  const list = filter ? projects.filter(p => p.id === filter) : projects;
    ...
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {list.map((p) => (
          ...
          >
            <div className="h-32 bg-[#2a2a31] flex items-center justify-center text-xs opacity-70">
              {/* Placeholder for screenshot */}
              {p.image ? <span>{p.title}</span> : <span>Screenshot Placeholder</span>}
            </div>
